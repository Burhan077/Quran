...
def fix_arabic(text: str) -> str:
    """Reshape and reorder Arabic so it renders properly in MoviePy."""
    reshaped_text = arabic_reshaper.reshape(text)   # connect letters
    bidi_text = get_display(reshaped_text)          # fix RTL order
    return bidi_text

# ---------------- STREAMLIT UI ----------------
st.title("Quran Video Generator (Arabic + English)")
    ...
    try:
        ...
        if not arabic_verses or not english_verses:
            ...
            for a, e in zip(arabic_verses, english_verses):
                arabic_fixed = fix_arabic(a["text"])
                verses.append({"arabic": arabic_fixed, "translation": e["text"]})
            ...
            if not audio_file:
                ...
                for i, verse in enumerate(verses):
                    # Arabic on top, translation below
                    full_text = f"{verse['arabic']}\n{verse['translation']}"
                    text_clip = TextClip(
                        full_text,
                        fontsize=40,
                        color="white",
                        font=FONT_PATH,
                        method="caption",
                        size=(background_clip.size[0], 150),
                        bg_color="black"
                    )
                    text_clip = (
                ...
                # Save
                output_video_path = os.path.join(BASE_DIR, f"surah_{surah_num_int}.mp4")
                final_clip.write_videofile(output_video_path, codec="libx264", audio_codec="aac")

                st.success(" Video generated successfully!")
                st.video(output_video_path)
                with open(output_video_path, "rb") as f:
                    st.download_button(" Download Video", f, file_name=f"surah_{surah_num_int}.mp4")
