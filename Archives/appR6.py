...
def index():
    if request.method == "POST":
        ...
        # Calculate how many times the background video needs to loop
        num_loops = int(audio_clip.duration // background_clip.duration) + 1

        # Create the background by repeating it manually
        clips = []
        ...
        for clip in clips[1:]:
            final_background_clip = final_background_clip + clip  # Manually concatenate

        # Trim the background video to match the audio length
        ...
        for i, verse in enumerate(verses):
            ...
            # Specify only 'text' for the 'TextClip' (no print_cmd argument)
            text_clip = TextClip(f"{arabic_text}\n{translation_text}",
                                 font=r"C:\Users\HP\Ideas\Al-Baghdadi1.2\backend\data\font\Amiri-Regular",
                                 color='white',
                                 bg_color='black',
                                 size=(background_clip.size[0], 100))  # Adjust the size of the box
                                 method='caption')
